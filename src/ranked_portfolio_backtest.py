...
TRADEABILITY_COLUMNS = {
    "min_price": ["entry_open", "event_close", "entry_price", "price", "close"],
    "min_dollar_volume": [
        "prior_20d_avg_dollar_volume", "avg_dollar_volume_20d", "dollar_volume_20d", "dollar_volume"
    ],
...
def join_feature_data(predictions: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
    ...
    available += [
        column for column in ("entry_date",) if column in right.columns and column not in left.columns
    ]
    return left.merge(right[keys + list(dict.fromkeys(available))], on=keys, how="left", validate="many_to_one")
...
def main() -> None:
    ...
    parser.add_argument(
        "--features-input", "--features", dest="features_input", default=None,
        help="Optional CSV/Parquet metadata merged on symbol and event_date",
    )
